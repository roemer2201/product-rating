...
- [x] **[S]** Zweiten Weg zum Hinzufügen eines Fotos anbieten: `PhotoManager`
      (`web/src/components/PhotoManager.tsx`) hatte nur den Aufnahme-Button mit
...
- [ ] **[M]** Auf einem echten iPhone prüfen: Installation, Scanner, Kamera-Upload, Sitzung überlebt Neustart, Safe-Area-Layout, dazu die Offline-Warteschlange samt Foto (dass ein `Blob` die IndexedDB übersteht, ist eine Zusage des Browsers – der Test-Ersatz gibt ihn als einfaches Objekt zurück und kann es deshalb nicht belegen). Stand 2026-08-24: Hinzufügen zum Home-Bildschirm, Scanner, Kamera-Upload und Safe-Area-Layout (Bedienelemente am unteren Rand kollidieren nicht mit dem Home-Indikator) bestätigt. Die Warteschlange verlor zunächst eine Erfassung, wenn iOS die App im Hintergrund beendete – Ursache und Behebung siehe HISTORY –, der Nachtest bestätigt die Behebung: die offline erfasste Erfassung überstand das Beenden und wurde danach automatisch übertragen. Offen bleibt allein die Erfassung mit Foto im selben Ablauf
- [x] **[S]** Nachtest der Warteschlange auf dem iPhone: offline erfassen, App aus dem App-Umschalter wischen, App neu starten – die Erfassung muss noch da sein und online übertragen werden. Bestätigt 2026-08-24
- [ ] **[S]** Anzeigen, wenn `navigator.storage.persist()` abgelehnt wurde: eine Warteschlange ohne Persistenzzusage kann iOS bei Platzmangel abräumen, und das sollte man erfahren, bevor man sich darauf verlässt
...
- [x] **[M]** `product-rating proxy-config --server nginx|apache|caddy|traefik`: dieselben vier Konfigurationen erzeugt statt als Vorlage, mit den Werten der Instanz (Hostname und Unterpfad aus `server.base_url`, Adresse aus `server.host`/`server.port`, Limit aus `uploads.max_file_size_mb`) und `--cert`/`--key` für das Zertifikat
- [ ] **[S]** Die erzeugten Konfigurationen sind nur gegen die Beispieldateien geprüft, nicht gegen die Webserver selbst – in der Entwicklungsumgebung ist keiner davon installiert. Beim ersten Einsatz `nginx -t`, `apache2ctl configtest` und `caddy validate` darüberlaufen lassen; das erzeugte Traefik-YAML ist immerhin gegen einen YAML-Parser geprüft
- [ ] **[S]** Für die Traefik-Compose-Labels gibt es keinen erzeugten Gegenpart: Labels gehören in die Compose-Datei des Nutzers, nicht in eine eigene Datei. Falls das je stört, wäre ein `--server traefik-labels` denkbar, das nur den `labels:`-Block ausgibt
