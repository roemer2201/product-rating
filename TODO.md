...
- [x] **[S]** Abgewiesene Origin-Prüfung diagnostizierbar machen: eigener Fehlercode `origin_rejected` statt `forbidden`, eigener Satz in der Oberfläche (der bisherige „Dafür fehlen die Rechte.“ schickt zur Nutzerverwaltung statt zur Konfiguration), Log-Zeile je Abweisung und ein Hinweis beim ersten Aufruf unter einer unbekannten Adresse
- [ ] **[S]** `proxy-config` könnte `server.base_url` auf Wunsch gleich setzen (`--apply` oder eine Rückfrage), statt den `--set`-Aufruf nur auf stderr zu nennen. Dagegen spricht, dass der Befehl bisher nichts schreibt, was ihm nicht ausdrücklich gesagt wurde; dafür, dass genau dieser vergessene Schritt die häufigste Störung nach dem Vorschalten eines Proxy ist
