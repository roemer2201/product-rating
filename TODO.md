...
- [ ] **[S]** Der Installationsdurchlauf lief ohne systemd (Container ohne PID 1): Verzeichnisse, Rechte, Secret, Migrationen, CLI, Start des Servers und `purge` sind geprüft, `systemctl start` und der Neustart beim Upgrade nicht. Der CI-Workflow holt genau das auf einem echten Runner nach. Am 23.08.2026 hat genau diese Lücke einen zweiten Härtungsfehler durchgelassen (`AF_NETLINK` fehlte, Dienst in der Neustartschleife bei `server.host = "0.0.0.0"`) – der Durchlauf sollte den Dienst deshalb nicht nur starten, sondern auch einmal mit Wildcard-Bindung
